...
function Answer(props){
    ...
    return (
        ...
        <div className='is-flex is-flex-direction-column is-justify-content-center mt-6'>
            { props.hint !== 2 && <button onClick={props.handleHint} className='hint-btn has-background-black-bis has-text-warning mb-3' data-testid='hintBtn'>Hint?</button>}
            ...
            <input className='answer-input input is-info' onChange={e => setIsInput(e.target.value)} value={isInput} data-testid='guessInput' type='text' id='answer' name='answer' placeholder='Answer goes here...'/>
            <button className='guess-btn has-background-info has-text-white mt-6 mb-3' type='submit' data-testid='guessBtn'>Guess</button>
            </form>
        </div>
    )
}
